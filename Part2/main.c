...
void score_combos(int score){
    char *scoringoptions[]= {"td+2", "td + fg", "td", "3pt fg", "s"};
    int vals[] = {8,7,6,3,2};
            ...
            for (int t = 0; t <= score/vals[2]; t++) {
                for (int fgl = 0; fgl <= score/vals[3]; fgl++) {
                    for (int sfty = 0; sfty <= score/vals[4]; sfty++) {
                        if (td2pt * vals[0] + td_fg * vals[1] + t * vals[2] + fgl * vals[3] + sfty * vals[4] == score) {
                            printf("%d %s, %d %s, %d %s, %d %s, %d %s\n", td2pt, scoringoptions[0], td_fg, scoringoptions[1], t, scoringoptions[2], fgl, scoringoptions[3], sfty, scoringoptions[4]);
                        }
                    }
                }
            }
